...
export default function ReportScreen({ reportId }: Props) {
  ...
  async function handlePdfDownload(): Promise<void> {
    ...
    setPdfLoading(true);
    try {
      await generateAndShareReport(report);
    } catch {
      // ইউজার share বাতিল করলে বা generation fail হলে silently ignore
    } finally {
      setPdfLoading(false);
    }
  }
    ...
    return (
      <SafeAreaView style={styles.safe}>
        ...
        <View style={styles.bottomBar}>
          ...
          <Pressable
            style={[styles.bottomPdfBtn, pdfLoading && styles.bottomBtnDisabled]}
          ...
          >
            {pdfLoading ? (
              ...
              <Ionicons name="download-outline" size={17} color={Colors.textOnPrimary} />
            )}
            <Text style={styles.bottomPdfText}>
              {pdfLoading
                ? t(ReportStrings.generatingPDF, lang)
                : t(ReportStrings.downloadPDF, lang)}
            </Text>
          </Pressable>
        </View>
      </SafeAreaView>
